...
The function `round_column()`round a selected column or a whole data frame to the specified number of decimal places (default 0). If no variables are infomed, then all numeric variables are rounded.
...
If the argument `drop` is set to `TRUE` then, only the new variable is keept and all others are dropped.
