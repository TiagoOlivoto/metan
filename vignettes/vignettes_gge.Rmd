...
# The GGE model
In AMMI analysis, the GEI effect is submitted to an SVP to construct biplots. In Genotype plus Genotype-vs-Environment interaction (GGE) model, instead of trying to separate G and GEI, a GGE biplot model keeps G and GEI together and partitions this mixture GGE into two multiplicative terms, according to the following model [@Yan2007; @Yan2003]

$$
{\bar y_{ij}} - \mu_j  = \sum\limits_{k = 1}^p {{\lambda _k}{\alpha_{ik}}} {\tau_{jk}}+ {\varepsilon _{ij}}
$$
where $\bar y_{ij}$ is the cell mean of genotype *i* in environment *j*; $\mu_j$ is the mean value in environment *j*; $i = 1, ... g$; $j = 1, ...e$ , *g* and *e* being the numbers of genotypes and environments, respectively; and *p* is the number of principal components (PC) used or retained in the model, with $t \le min(e,g ??? 1)$. The, model above may be described as modeling the deviations of the cell means from the environment means as a sum of PCs, each of which is the product of a genotype score ($\alpha_{ik}$), an environment score ($\tau_{jk}$), and a scale factor (the singular value, $\lambda_k$)
...
The generic function `plot()` is used to generate a biplot using as input a fitted model of class `gge`. The biplot type is contrled by the argument `type` in the function.  Ten biplots type are available according to {@Yan2003}.
...
In this material, for each biplot type, two graphics are produced. One with the default settings and the other to show some graphical options of the function. Please, keep in mind that each biplot interpretation require different scaling and centering methods. In this material we will used the default setting in the model stored in `gge_model`. It is suggested to see 
...
## Biplot type 8: Ranking gentoypes.
