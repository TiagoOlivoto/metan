...
$$
where $\hat y_{ij}$ is the mean yield of genotype *i* in environment *j*, $i = 1, ... g; j = 1, ...e$ being *g* and *e*  the numbers of genotypes and environments, respectively; $\mu_j$ is the grand mean; $\alpha_i$ is the main effect of the genotype *i*; $\beta_j$ is the main effect of the environment *j*, and $\phi_{ij}$ is the interaction effect between genotype *i* and environment *j*. Subjecting the $\phi_{ij}$ to Singular Value Decomposition (SVD) results in the AMMI model. The deletion of $\alpha_i$ allows the variation explained by this term to be absorved into the $\phi_{ij}$ term. In the Genotype plus Genotype-vs-Environment interaction (GGE) model the $\alpha_i$ term is is deteled from the above model and then the environment-centered data matrix, $\phi_{ij}$, is subjected to SVD [@Yan2007; @Yan2003]. Explicitly, we have
...
# Singular Value Partitioning methods
