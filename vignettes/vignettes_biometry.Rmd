...
library(tibble) # Used for manipulating rownames
...
The following code creates a ggplot-based graphic showing the confidence interval estimated in the object **CI**

```{r, fig.width=6, fig.height = 4, fig.align="center"}
ggplot(CI, aes(x = reorder(Pair, Corr), y = Corr))+
  geom_errorbar(aes(ymax = UL, ymin = LL, width = 0.5))+
  geom_point(col = "black", fill = "red", size = 3, shape = 21)+
  coord_flip()+
  ...
  ggtitle("95% CI for Pearson's correlation coefficient",
          subtitle = "n = 156")
  labs(x = "Pairwise combinations", y = "Pearson's correlation coefficient")
```
...
It is possible to indicate the variables from the dataset to compute the distances. To do that is easy. You should only provide a comma-separated list of unquoted variable names after the .data argument. For example, to compute the distances between the genotypes based on the variables NKR, TKW, and NKE, the following arguments should be used.
...
When `selvar = TRUE` is used, an algorithm for variable selction is implemented. See `?clustering` for more details.
```{r, fig.height = 5, fig.width = 5.5, fig.align = "center", message=FALSE, warning=FALSE}
...
The function `pairs_mantel()` may be used to check the relationships between the distance matrices when the clustering is performed for each level of a grouping factor. In this example, we have four distance matrices correponding to four environments.
```{r, fig.height = 5, fig.width = 5.5, fig.align = "center"}
