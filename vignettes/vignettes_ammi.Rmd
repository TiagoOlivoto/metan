...
The genotype-vs-environment interaction was highly significant. So we'll proced with the AMMI analysis.
...
```{r echo = TRUE}
AMMI_model = data_ge %>% performs_ammi(ENV, GEN, REP, GY, verbose = FALSE)
    ```
...
```{r}
waas_index <- data_ge %>% waas(ENV, GEN, REP, GY, verbose = FALSE)
```
...
```{r echo = TRUE}
WAASratio = wsmp(waas_index, increment = 5)
