...
The `metan` package provides tools for computing several world-known stability statistics. In the latest version (v 1.2.0), the following stability methods are implemented in their respective functions.

* Parametric methods
   * AMMI analysis with `performs_ammi()`.
   * GGE biplot analysis with `gge()`.
   * BLUP prediction for multi-environment trials with `waasb()`.
   * AMMI-based stability statistics with `AMMI_indexes()`.
   * BLUP-based stability indexes with `Resende_indexes()`.
   * Joint regression analysis [@Eberhart1966] using `ge_reg()`.
   * Genotypic confidence index [@Annicchiarico1992] using `Annicchiarico()`.
   * Modified genotypic confidence index [@Schmildt2011] using `Schmildt()`.
   * Ecovalence [@Wricke1965] using `ecovalence()`.
   * Stability analysis and environmental stratification [@murakami2004] using `ge_factanal()`.
   * Shukla's stability variance [@Shukla1972] using `Shukla()`.
   * Weighted Average of Absolute Scores [@Olivoto2019] using `waas()` (fixed-effect model) and `waasb()` (mixed-effect model).

* Non-parametric methods
   * Superiority index [@Lin1988], using the function `superiority()`
   * Fox's stability statistic [@Fox1990] with `Fox()`.
   * Geometric adaptability index [@Shahbazi2019] with `gai()`.
   * Huehn's stability statistic [@Huehn1979] with `Huehn()`.
   * Thennarasu's stability statistics [@Thennarasu1995] with `Thennarasu()`
   

The easiest way to compute the above-mentioned stability indexes is by using the function `ge_stats()`. This is a wrapper that basically returns a summary of each method. If you are looking for more details from each method like `plot()` and `print()`, I suggest computing that methods using their own function.
...
# BLUP model
The implementation of linear-mixed effect models to predict the response variable in MET is made with the function `waasb()`. By default, genotype and genotype-vs-environment interaction are assumed to have random effects. Use the argument `random` to change this default. In the following example the model is fitted to all numeric variables in `data_ge2` quickly using the [select helper](https://tiagoolivoto.github.io/metan/articles/vignettes_helper.html#select-helpers) `everything()` in the argument `resp`.

```{r warning=FALSE, message=FALSE}
model2 <- waasb(data_ge, ENV, GEN, REP, everything())
```

The mixed-effect model fitted in `model2` has many outputs. The easitest way  For example, we can easily obtain the Likelihood Ration Test for random effects, the variance components, and the BLUPs for genotypes as follows:

```{r}
get_model_data(model2) %>% print_table()
```
...
To compute all the stability statistics at once, we can use the function `ge_stats()`. Get the results easely by using `get_model_data()`.

```{r}
stat_ge <- ge_stats(data_ge, ENV, GEN, REP, GY)
get_model_data(stat_ge) %>% print_table()
```


# Selection based on multiple traits

The multi-trait stability index (MTSI) was proposed by @Olivoto2019a and is used for simultaneous selection considering mean performance and stability (of several traits) in the analysis of METs using both fixed and mixed-effect models.
