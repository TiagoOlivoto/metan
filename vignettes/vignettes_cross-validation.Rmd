...
AMMI0 = cv_ammi(data_ge, ENV, GEN, REP, GY, naxis = 0, nboot = 500, nrepval = 2)
AMMI2 = cv_ammi(data_ge, ENV, GEN, REP, GY, naxis = 2, nboot = 500, nrepval = 2)
AMMI9 = cv_ammi(data_ge, ENV, GEN, REP, GY, naxis = 9, nboot = 500, nrepval = 2)
```

## AMMI0 to AMMIF
```{r eval = TRUE, fig.height = 5, fig.width = 10, fig.align = "center" }
AMMIF = cv_ammif(data_ge, ENV, GEN, REP, GY, nboot = 500, nrepval = 2)
```

# Cross-validation for BLUP prediction
## Genotype and genotype-vs-environment as random effects
```{r eval = TRUE, fig.height = 5, fig.width = 10, fig.align = "center" }
BLUP_g = cv_blup(data_ge, ENV, GEN, REP, GY, nboot = 500, nrepval = 2, random = "gen")
```

## Environment, replication-whitin-environment and interaction as random effects
```{r eval = TRUE, fig.height = 5, fig.width = 10, fig.align = "center" }
BLUP_e = cv_blup(data_ge, ENV, GEN, REP, GY, nboot = 500, nrepval = 2, random = "env")
```

## A random model (all terms as random effects)
```{r eval = TRUE, fig.height = 5, fig.width = 10, fig.align = "center" }
BLUP_ge = cv_blup(data_ge, ENV, GEN, REP, GY, nboot = 500, nrepval = 2, random = "all")
```
