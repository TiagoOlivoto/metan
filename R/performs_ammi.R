...
performs_ammi <- function(ENV, GEN, REP, Y) {
    ...
    df <- fortify(model)
    df <- data.frame(fitted = df[, 8], resid = df[, 9], stdres = df[, 10])
    ...
    MGEN <- data.frame(type = "GEN", Y = apply(OUTMED, 1, mean), MSCORES)
    MENV <- data.frame(type = "ENV", Y = apply(OUTMED, 2, mean), NSCORES)
