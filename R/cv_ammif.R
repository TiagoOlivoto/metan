...
cv_ammif <- function(.data, env, gen, rep, resp, nboot = 100,
    ...
    for (y in 1:naxisvalidation) {
        ...
        for (b in 1:nboot) {
            ...
            testing <- suppressWarnings(dplyr::anti_join(data,
                                                         modeling, by = c("ENV", "GEN", "REP", "Y", "ID"))) %>%
            ...
            MEDIAS %<>% mutate(Ypred = Y - residual, ResAMMI = ((MGEN %*%
                                                                     s$u[, 1:NAXIS]) * (MENV %*% s$v[, 1:NAXIS])) %*%
                                   s$d[1:NAXIS], YpredAMMI = Ypred + ResAMMI, testing = testing$Y,
                               error = YpredAMMI - testing, errrorAMMI0 = Ypred -
                                   testing)
        ...
        } else AMMIval[[sprintf("AMMI%.0f", NAXIS)]] <- RMSPDres[,
                                                                 1]
        NAXIS <- NAXIS - 1
        initial <- initial
    }
    ...
    RMSPDmean <- RMSPD %>% dplyr::group_by(MODEL) %>% dplyr::summarise(mean = mean(RMSPD),
                                                                       sd = sd(RMSPD), se = sd(RMSPD)/sqrt(n()), Q2.5 = quantile(RMSPD,
                                                                                                                                 0.025), Q97.5 = quantile(RMSPD, 0.975)) %>% arrange(mean)
    return(structure(list(RMSPD = RMSPD, RMSPDmean = RMSPDmean,
