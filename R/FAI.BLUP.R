FAI.BLUP <- function(x, DI, UI, show = TRUE, SI = NULL, mineval = 1){
  ...
  if(!is.null(ngs)){
    selection.diferential <- lapply(1:IN, function(i){cbind(pos.var.factor[,2], ((colMeans(means.factor[names(ideotype.rank[[i]])[1:ngs],]) - colMeans(means.factor))/colMeans(means.factor))*100)})
    for(i in 1:IN){
