...
values = c(shape.env, shape.gen)) + geom_segment(data = x$model,
aes(x = 0, y = 0, xend = PC1, yend = PC2, size = type,
color = type, group = type), arrow = arrow(length = unit(0.15,
"cm"))) + scale_fill_manual(labels = leg.lab,
...
alpha = line.alpha) + geom_point(size = size.shape,
stroke = size.bor.tick, aes(fill = type), alpha = col.alpha) +
geom_segment(data = x$model, aes(x = mean, y = 0,
xend = Y, yend = PC1, size = type, color = type,
group = type), arrow = arrow(length = unit(0.15,
"cm"))) + scale_shape_manual(labels = leg.lab,
