...
plot_scores <- function(x, type = 1, polygon = FALSE, file.type = "pdf", export = FALSE,
    ...
    if (type == 1) {
        ...
        p1 <- ggplot(x$model, aes(PC1, PC2, shape = type, fill = type)) + geom_vline(xintercept = 0,
                                                                                     linetype = line.type, color = col.line, size = size.line, alpha = line.alpha) +
            geom_hline(yintercept = 0, linetype = line.type, color = col.line, size = size.line,
                       alpha = line.alpha) + geom_segment(data = x$model, aes(x = 0, y = 0,
                                                                              xend = PC1, yend = PC2, size = type, color = type, group = type), arrow = arrow(length = unit(0.15,
                                                                                                                                                                            "cm"))) + geom_point(size = size.shape, stroke = size.bor.tick, aes(fill = type),
            ...
            labs(x = paste(x.lab), y = paste(y.lab)) + scale_x_continuous(limits = x.lim,
                                                                          breaks = x.breaks) + scale_y_continuous(limits = y.lim, breaks = y.breaks) +
            scale_color_manual(name = "", values = c(col.segm.gen, col.segm.env),
                               theme(legend.position = "none")) + scale_size_manual(name = "", values = c(size.segm.line,
        ...
        if (polygon == TRUE) {
            ...
            while (is.na(indice[i + 1]) == FALSE) {
                m <- (coordgenotype[indice[i], 2] - coordgenotype[indice[i + 1], 2])/(coordgenotype[indice[i],
                                                                                                    1] - coordgenotype[indice[i + 1], 1])
                mperp <- -1/m
                c2 <- coordgenotype[indice[i + 1], 2] - m * coordgenotype[indice[i +
                                                                                     1], 1]
                xint <- -c2/(m - mperp)
                xint <- ifelse(xint < 0, min(coordenviroment[, 1], coordgenotype[,
                                                                                 1]), max(coordenviroment[, 1], coordgenotype[, 1]))
                yint <- mperp * xint
            ...
            p1 <- p1 + geom_segment(aes(x = X1, y = X2), xend = 0, yend = 0, linetype = 2,
                                    size = size.segm.line, color = "black", data = segs, inherit.aes = FALSE) +
                geom_polygon(data = gen[indice, ], fill = NA, col = col.gen, linetype = 2)
        ...
        if (file.type == "tiff") {
            if (is.null(file.name)) {
                tiff(filename = "PC1 x PC2.tiff", width = width, height = height,
    ...
    if (type == 2) {
        ...
        mean <- mean(x$model$Y)
        p2 <- ggplot2::ggplot(x$model, aes(Y, PC1, shape = type, fill = type)) + geom_vline(xintercept = mean(x$model$Y),
                                                                                            linetype = line.type, color = col.line, size = size.line, alpha = line.alpha) +
            geom_hline(yintercept = 0, linetype = line.type, size = size.line, color = col.line,
                       alpha = line.alpha) + geom_segment(data = x$model, aes(x = mean, y = 0,
                                                                              xend = Y, yend = PC1, size = type, color = type, group = type), arrow = arrow(length = unit(0.15,
                                                                                                                                                                          "cm"))) + geom_point(size = size.shape, stroke = size.bor.tick, aes(fill = type),
                                                                                                                                                                                               alpha = col.alpha) + scale_shape_manual(labels = leg.lab, values = c(shape.gen,
                                                                                                                                                                                                                                                                    shape.env)) + scale_fill_manual(labels = leg.lab, values = c(col.gen,
        ...
        if (file.type == "tiff") {
            if (is.null(file.name)) {
                tiff(filename = "GY x PC1.tiff", width = width, height = height, units = "in",
    ...
    if (type == 3) {
        ...
        if (class == "waasb") {
            ...
            if (is.null(y.lim) == F) {
                ...
                y.lim <- c(min(x$model$WAASB) - (min(x$model$WAASB) * axis.expand -
                                                     min(x$model$WAASB)), max(x$model$WAASB) + (max(x$model$WAASB) *
                                                                                                    axis.expand - max(x$model$WAASB)))
            }
            m1 <- mean(x$model$Y)
            ...
            p3 <- ggplot2::ggplot(x$model, aes(Y, WAASB, shape = type, fill = type)) +
                geom_vline(xintercept = m1, linetype = line.type, color = col.line,
                           size = size.line, alpha = line.alpha) + geom_hline(yintercept = m2,
                                                                              linetype = line.type, color = col.line, size = size.line, alpha = line.alpha) +
                geom_point(size = size.shape, stroke = size.bor.tick, aes(fill = type),
                           alpha = col.alpha) + scale_shape_manual(labels = leg.lab, values = c(shape.gen,
                                                                                                shape.env)) + scale_fill_manual(labels = leg.lab, values = c(col.gen,
                ...
                labs(x = paste(x.lab), y = paste(y.lab)) + scale_x_continuous(limits = x.lim,
                                                                              breaks = x.breaks) + scale_y_continuous(limits = y.lim, breaks = y.breaks) +
                annotation_custom(I) + annotation_custom(II) + annotation_custom(III) +
        ...
        if (file.type == "tiff") {
            if (is.null(file.name)) {
                tiff(filename = "GY x WAAS.tiff", width = width, height = height,
                     units = "in", compression = "lzw", res = resolution)
            } else tiff(filename = paste0(file.name, ".tiff"), width = width, height = height,
                        units = "in", compression = "lzw", res = resolution)
            plot(p3)
    ...
    if (type == 4) {
        ...
        p4 <- ggplot2::ggplot(data, aes(x = envPC1, y = nominal, group = GEN)) +
            geom_line(size = 1, aes(colour = GEN), data = subset(data, envPC1 %in% c(max(envPC1), min(envPC1)))) +
            geom_point(aes(x = envPC1,  y = minim), data = subset(data, GEN == data[1, 2])) +
            ggrepel::geom_label_repel(data = subset(data, envPC1 == min(envPC1)),
                                      aes(label = GEN, fill = GEN), size = size.tex.pa, color = "white", force = repulsion, segment.color = "#bbbbbb") +
            ggrepel::geom_text_repel(aes(x = envPC1, y = minim, label = ENV), size = size.tex.pa, force = repulsion, data = subset(data,
            ...
            scale_y_continuous(limits = y.lim, breaks = y.breaks) + labs(x = paste(x.lab),
            y = y.lab)
        ...
        if (file.type == "tiff") {
            if (is.null(file.name)) {
                tiff(filename = "Adaptative reponses (AMMI1 model).tiff", width = width,
                  height = height, units = "in", compression = "lzw", res = resolution)
            } else tiff(filename = paste0(file.name, ".tiff"), width = width, height = height,
                units = "in", compression = "lzw", res = resolution)
            plot(p4)
            dev.off()
