...
#'
#' @param x The \code{WAASratio.AMMI object}
...
plot.WAASratio.AMMI <- function(x, type = 2, export = FALSE, file.type = "pdf", file.name = NULL,
    ...
    stop("'plot.WAASratio.AMMI' is deprecated. Please, use 'plot.wsmp' instead")
    data <- x
    if (type == 1) {
        ...
        if (export == F | FALSE) {
            ...
            gplots::heatmap.2(data$hetdata, dendrogram = "both", col = colfunc(nrow(subset(data$WAASY,
                ...
                key.ylab = "", key.par = list(mgp = c(1.5, 0.5, 0), mar = c(3, 0.2,
                  0.2, 0.2)), lhei = c(1, 6), xlab = x.lab, ylab = y.lab, margins = margins)
        ...
        if (file.type == "tiff") {
            if (is.null(file.name)) {
                tiff(filename = "Heat Ranks PCA.tiff", width = width, height = height,
    ...
    if (type == 2) {
        ...
        if (export == F | FALSE) {
            ...
            gplots::heatmap.2(data$hetcomb, dendrogram = "row", col = colfunc(nrow(subset(data$WAASY,
                ...
                key.xlab = key.lab, key.ylab = "", key.par = list(mgp = c(1.5, 0.5,
                  0), mar = c(3, 0.2, 0.2, 0.2)), lhei = c(1, 6), xlab = x.lab, ylab = y.lab,
                cexCol = size.lab, cexRow = size.lab, margins = margins)
        ...
        if (file.type == "tiff") {
            if (is.null(file.name)) {
                tiff(filename = "Heat map Ranks WAAS-GY.tiff", width = width, height = height,
                  units = "in", compression = "lzw", res = resolution)
            } else tiff(filename = paste0(file.name, ".tiff"), width = width, height = height,
                units = "in", compression = "lzw", res = resolution)
