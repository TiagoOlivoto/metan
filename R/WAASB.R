WAASB = function(data, resp, gen, env, rep, random = "gen",
  ...
  if (random == "all") {
    ...
    GV = as.numeric(random[3, 2])
    GEV = as.numeric(random[4, 2])
    RV = as.numeric(random[6, 2])
    FV = GEV + GV + EV + RV
    h2g = GV/FV
    ...
    Ponderado = data.table::transpose(as.data.frame(sapply(t_WAAS[,
                                                                  -ncol(t_WAAS)], weighted.mean, w = t_WAAS$Percent)))
    ...
    WAASAbs = data.table::setDT(WAASAbs)[, `:=`(OrResp, rank(-Y)),
                                         by = type][]
    WAASAbs = data.table::setDT(WAASAbs)[, `:=`(OrWAASB, rank(WAASB)),
                                         by = type][]
    WAASAbs = data.table::setDT(WAASAbs)[, `:=`(OrPC1, rank(abs(PC1))),
                                         by = type][]
    ...
    max = paste0(round(max$Y, 4), " (Genotype ", max$GEN, " in ", max$ENV,
                 " )")
    Details = list(WgtResponse = weight.response, WgtWAAS = weight.WAAS,
                   Ngen = Ngen, Nenv = Nenv, OVmean = mean, Min = min, Max = max,
                   MinENV = MinENV, MaxENV = MaxENV, MinGEN = MinGEN, MaxGEN = MaxGEN)
    Details = do.call(rbind.data.frame, Details)
    names(Details) = "Values"
    Details = dplyr::mutate(Details, Parameters = c("WgtResponse",
                                                    "WgtWAAS", "Ngen", "Nenv", "OVmean", "Min", "Max", "MinENV",
    ...
    selectioNenv = suppressMessages(dplyr::mutate(selectioNenv, BLUPe = left_join(blups,
                                                                                  blupENV %>% select(Code, BLUPe))$BLUPe, ggee = BLUPge + BLUPg +
