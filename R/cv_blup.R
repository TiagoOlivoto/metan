...
cv_blup <- function(.data, env, gen, rep, resp, nboot = 100,
                    nrepval, random = "gen", verbose = TRUE) {
    ...
    for (b in 1:nboot) {
        ...
        modeling <- do.call(rbind, lapply(tmp, function(x) {
            ...
            x %>% dplyr::group_by(!!enquo(gen)) %>% dplyr::filter(unique(data$REP) %in%
                                                                      c(X2))
        })) %>% as.data.frame()
        rownames(modeling) <- modeling$ID
        testing <- suppressWarnings(dplyr::anti_join(data, modeling,
                                                     by = c("ENV", "GEN", "REP", "Y", "ID"))) %>% arrange(ENV,
                                                                                                          GEN, REP) %>% as.data.frame()
        ...
        if (random == "env") {
            model <- suppressWarnings(suppressMessages(lme4::lmer(Y ~
                                                                      GEN + (1 | ENV/REP) + (1 | GEN:ENV), data = modeling)))
        }
        if (random == "all") {
            model <- suppressWarnings(suppressMessages(lme4::lmer(Y ~
                                                                      1 + (1 | GEN) + (1 | ENV/REP) + (1 | GEN:ENV),
                                                                  data = modeling)))
        }
    ...
    RMSPDres <- dplyr::mutate(RMSPDres, MODEL = MOD) %>% dplyr::select(MODEL,
                                                                       everything())
