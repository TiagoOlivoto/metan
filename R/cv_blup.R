...
#' combinations (genotype x environment) with R - 1 replications.
#'  The 'validation' set has the remaining replication.
#'  The estimated values are compared with the 'validation' data
#' and the Root Means Square Prediction Difference (Olivoto et al. 2019) is computed.
...
#' design (Patterson and Williams, 1976) is employed. See how fixed and random effects are
#' considered, see the section \strong{Details}.
#' @param nboot The number of resamples to be used in the cross-validation. Defaults to 100.
#' @param random The effects of the model assumed to be random. See \strong{Details} for more
#' information.
#' @param verbose A logical argument to define if a progress bar is shown.
#' Default is \code{TRUE}.
#' @details Six models may be fitted depending upon the values in \code{block} and \code{random} arguments.

#' *  \strong{Model 1:} \code{block = NULL} and \code{random = "gen"} (The default option).
#' This model considers a Randomized Complete Block Design assuming genotype and
#' genotype-vs-environment as random effects. Environment and blocks nested  within environments
#' are treated as fixed factors.
#'
#' *  \strong{Model 2:} \code{block = NULL} and \code{random = "env"}.
#' This model considers a Randomized Complete Block Design treating environment,
#' genotype-vs-environment, and blocks-within-environments as random factors. Genotypes
#' are assumed to be fixed factors.
#'
#' *  \strong{Model 3:} \code{block = NULL} and \code{random = "all"}.
#' This model considers a Randomized Complete Block Design assuming all effects (genotypes, environments,
#' genotype-vs-environment interaction and blocks nested within environments) as random.
#'
#' *  \strong{Model 4:} \code{block != NULL} and \code{random = "gen"}.
#' This model considers an alpha-lattice design assuming genotype, genotype-vs-environment
#' interaction, and incomplete block nested within replicates as random to make use of inter-block
#' information (Mohring et al., 2015). Complete replicates nested within environments and environments
#' are treated as fixed factors.
#'
#' *  \strong{Model 5:} \code{block != NULL} and \code{random = "env"}.
#' This model considers an alpha-lattice design assuming genotype as fixed. All other sources of
#' variation (environment, complete replicates nested within environments, and incomplete blocks nested
#' within replicates) as treated as random factors.
#'
#' *  \strong{Model 6:} \code{block != NULL} and \code{random = "all"}.
#' This model considers an alpha-lattice design assuming all effects, except the intercept, as random
#' factors.
#'
#'
#' @return
...
#' }
#'
cv_blup <- function(.data, env, gen, rep, resp, block = NULL, nboot = 5, random = "gen", verbose = TRUE) {
    if(missing(block)){
        data <- .data %>%
