...
predict.gge = function(object, naxis = 2, output = "wide", ...){
  if(naxis > min(dim(object$coordenv))){
    stop("The number of principal components cannot be greater than min(g, e), in this case ", min(dim(object$coordenv)))
  }
  # SVP
  if(object$svp == "environment"){
    pred = (object$coordgen[,1:naxis] * (object$d)) %*% t(object$coordenv[,1:naxis])
  }
  if(object$svp == "genotype"){
    pred = (object$coordgen[,1:naxis] %*% t(object$coordenv[,1:naxis]* (object$d)))
  }
  if(object$svp == "symmetrical"){
    pred = (object$coordgen[,1:naxis] %*% t(object$coordenv[,1:naxis]))
  }
  # Scaling
  ...
  # Centering
  if(object$centering == "global"){
    pred = pred + object$grand_mean
  }
  if(object$centering == "environment"){
    pred = sweep(pred, 2, object$mean_env, FUN = "+")
  }
  if(object$centering == "double"){
    for (i in 1:nrow(pred)) {
      for (j in 1:ncol(pred)) {
        pred[i, j] <- pred[i, j] - object$grand_mean + object$mean_env[j] + object$mean_gen[i]
      }
    }
  }
  ...
  if(output == "long"){
    return(pred %>% as.data.frame() %>%
             rownames_to_column("GEN") %>%
             gather(- GEN, key = "ENV", value = "Y") %>%
